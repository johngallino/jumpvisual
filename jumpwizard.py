...
#PATH = os.path.join(os.path.expanduser('~'), 'Desktop', 'locked')
print(PATH)
#PATH = 'photographers/'
global canWrite
canWrite = True
try:
    file = open(os.path.join(PATH, 'test.jmp'), 'w')
except:
    canWrite = False

#print("canWrite is " + str(canWrite))
...
def checkjumpdb():
    ...
    if not found:
        ...
        #dberror.title("Missing jump.db file")
        def shutdown():
                raise SystemExit
...
#        self.manbox.grid(row=1, column=2, sticky=tk.W, padx=10)
        ...
        #tk.Label(self, text=("ERROR: The program does not have write access to the folder\n" + PATH + "\nYou cannot proceed"), fg="RED").grid(row=10, column=0)
...
class NYTowns(tk.Frame):
    ...
    def __init__(self, parent, user, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)
        app.img = tk.PhotoImage(file="s_ny.pbm") 
...
class FinalFrame(tk.Frame):
    ...
    def __init__(self, parent, User, *args, **kwargs):
        ...
        #update sidebar
        app.sidebar.destroy()
        app.sidebar = tk.Label(app.leftframe, image=app.baseimage)
        app.sidebar.grid(row=0, column=0, sticky=tk.N + tk.S)
        app.version = tk.Label(app.leftframe, text=version, justify=tk.LEFT)
        app.version.place(x=1, y=1, anchor=tk.NW)
        ...
        tk.Label(self, text=str(PATH), fg="BLUE").grid(row=2, column=0, columnspan=2, sticky='ew')
        ...
        self.finlabelbad = tk.Label(report, fg="RED", text=("Uh oh! Writing to ? didn't work. I have exported to C:\ instead. Please check there!.", (PATH,)), anchor=tk.W)
...
class Wizard(tk.Tk):
    ...
    def writeError_pop(self):
            window = tk.Toplevel()
            center(window)
            window.title = "Write access error"
            f = tk.Frame(window)
            f.grid(row=0, column=0)
            tk.Label(f, text="This program does not have write permissions for the folder").grid(row=0, column=0, padx=20, pady=(10,0))
            tk.Label(f, text=PATH).grid(row=1, padx=20, pady=5)
            tk.Label(f, text="Please enter a new path to export the report to:").grid(row=2, column=0, pady=(0,10), padx=20)
            newPath = tk.StringVar()
            newPath.set(os.path.expanduser('~'))
            pathbox = tk.Entry(window, textvariable=newPath, width=50)
            pathbox.grid(row=3, pady=(0,10))
            ...
            def pullPath():
                ...
                print("canWrite is now " + str(canWrite))
                self.current_frame = self.frames[0](self, user, padx=30, pady=30, width=400)        
                self.current_frame.grid(row=0,column=1)
                
            tk.Button(window, text="OK", command=pullPath).grid(row=4, pady=10, padx=20, ipadx=15)
    ...
    def generate(self, user):
        ...
        file.write(user.hometown + "\n")
        file.write(user.homestate + "\n")
        file.write(user.zip + "\n")
        file.write(user.birthday + "\n")
        file.write(user.faa_num + "\n")
        file.write(user.abilities + "\n")
        file.write(user.emer_name + "\n")
        file.write(user.emer_relation + "\n")
        file.write(user.emer_cell + "\n")
