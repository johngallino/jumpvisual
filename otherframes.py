...
class NJTowns(tk.Frame):
	...
	def __init__(self, parent, user, *args, **kwargs):
		...
		j.c.execute("SELECT upper(county_name) ||' | ' || city FROM UScities WHERE county_name=?", (county,))
...
class NYCounties(tk.Frame):
	...
	def __init__(self, parent, user, *args, **kwargs):
		...
		tk.Label(transfer, text="the photographer's Coverage Zone:").grid(row=0, column=3, sticky=tk.W)
...
class FinalFrame(tk.Frame):
	...
	def __init__(self, parent, user, *args, **kwargs):
		...
		self.finlabelgood = tk.Label(report, fg="GREEN", text="Team Member succesfully added to database! You may now close this wizard.", anchor=tk.W)
